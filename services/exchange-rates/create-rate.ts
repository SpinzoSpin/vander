...
export async function getPhpToUsdRate(): Promise<number> {
  const url =
    "https://api.pro.coins.ph/openapi/quote/v1/ticker/price?symbol=USDTPHP"

  const response = await fetch(url)
  if (!response.ok) {
    throw new ApiError(
      `Coins.ph API error: ${response.status} ${response.statusText}`,
      502
    )
  }
  ...
  if (isNaN(usdtToPhpRate) || usdtToPhpRate <= 0) {
    throw new ApiError(
      `Invalid price value from Coins.ph API: ${data.price}`,
      502
    )
  }
...
export async function createExchangeRate(manualRates?: ManualRates) {
  ...
  const phpToUsdtSpinzoRate =
    spinzoFee / (usdtToPhpReferenceRate * usdtToPhpReferenceRate)
  const phpToUsdtGicRate =
    gicFee / (usdtToPhpReferenceRate * usdtToPhpReferenceRate)
  const phpToUsdtRate = roundToSixDecimals(
    phpToUsdtReferenceRate - phpToUsdtSpinzoRate - phpToUsdtGicRate
  )
  ...
  const phpToUsdtSpreadPercentage = roundToTwoDecimals(
    phpToUsdtReferenceRate > 0
      ? (phpToUsdtDiff / phpToUsdtReferenceRate) * 100
      : 0
  )

  // 4. Compute Crypto to Fiat (USDT -> PHP)
  // Selling USDT: user gets LESS PHP than market. Fee is in PHP.
  const usdtToPhpRate = roundToSixDecimals(
    usdtToPhpReferenceRate - spinzoFee - gicFee
  )
  ...
  const usdtToPhpSpreadPercentage = roundToTwoDecimals(
    usdtToPhpReferenceRate > 0
      ? (usdtToPhpDiff / usdtToPhpReferenceRate) * 100
      : 0
  )
