...
function formatTransactionMessage(t: any): string {
    ...
    const statusEmoji = {
        ...
        crypto_arrival: "🪙 Crypto Arrived"
    }[t.status as string] || t.status;

    // Generate Admin Panel Link
    const appUrl = process.env.APP_URL || "https://exc-admin-stag.spinzo.io";
    const linkPath = isFiatToCrypto ? "/dashboard/operations/fiat-to-crypto" : "/dashboard/operations/crypto-to-fiat";
    ...
    if (isFiatToCrypto) {
        ...
        detailsSection = `<b>Platform Deposit Bank (Exchanger):</b>${depositBankStr}\n` +
        ...
        detailsSection = `<b>Platform Deposit Treasury Wallet:</b>\n<code>${t.treasury?.wallet_address || "N/A"}</code>\n\n` +
                         `<b>Payout Bank Account (Lotto):</b>${payoutBankStr}\n`;
    }

    return `<b>🆕 Transaction Notification</b>\n\n` +
...
function getReplyMarkupForStatus(t: any) {
    ...
    if (t.status === "pending") {
        ...
        if (isFiatToCrypto) {
            ...
            inline_keyboard.push([
                { text: "📥 Crypto Arrived", callback_data: `tx_${t.id}_crypto_arrival` }
            ]);
        }
    } else if (t.status === "confirmed" || t.status === "fiat_arrival" || t.status === "crypto_arrival") {
        inline_keyboard.push([
...
export async function sendTelegramNotification(transactionId: number) {
    ...
    if (chatIds.length === 0) {
        console.warn("No allowed Telegram chat IDs found in TELEGRAM_CHAT_ID env. Skipping notification.");
        return;
    }
    ...
    try {
        ...
        if (!t) {
            console.error(`Transaction with ID ${transactionId} not found for Telegram notification.`);
            return;
        }
        ...
        if (fs.existsSync(LOCK_FILE)) {
            ...
            if (!isNaN(oldPid)) {
                try {
                    ...
                    console.warn(`[Bot Lock] Another bot listener instance is already running with PID ${oldPid}. Skipping bot listener start.`);
                    return false;
                } catch (e: any) {
                    if (e.code !== "ESRCH") {
                        console.error(`[Bot Lock] Error checking process status for PID ${oldPid}:`, e);
                        return false;
                    }
                }
            }
...
export function startTelegramBot() {
    if (!BOT_TOKEN) {
        console.warn("Telegram BOT_TOKEN is not configured. Background bot listener disabled.");
        return;
    }
    ...
    console.log(`[${new Date().toISOString()}] Starting Telegram bot listener with TELEGRAM_CHAT_ID filter (PID: ${process.pid})...`);
    ...
    async function pollUpdates() {
        try {
            ...
            if (resData.ok && resData.result.length > 0) {
                for (const update of resData.result) {
                    ...
                    if (update.message) {
                        ...
                        if (text.startsWith("/start") || text.startsWith("/register") || text.startsWith("/help")) {
            ...
            errorCount++;
            const isTimeout = e.message?.includes("fetch failed") || e.code === "ETIMEDOUT" || e.code === "ECONNRESET";
            if (isTimeout) {
                console.warn(`[Telegram Bot] Connection timeout/failed (attempt ${errorCount}). Retrying...`);
            } else {
...
async function handleCallbackQuery(callbackQuery: any) {
    ...
    if (!isUserAllowed && !isChatAllowed) {
        await answerCallbackQuery(callbackQueryId, "❌ Unauthorized: You do not have permissions to manage transactions.");
        return;
    }
    ...
    try {
        ...
        if (!allowed || !allowed.includes(newStatus)) {
            await answerCallbackQuery(callbackQueryId, `⚠️ Cannot transition from "${t.status}" to "${newStatus}".`);
            return;
        }
        ...
        if (!updatedTransaction) {
            await answerCallbackQuery(callbackQueryId, "❌ Failed to reload updated transaction.");
            return;
        }

        await answerCallbackQuery(callbackQueryId, `✅ Status updated to: ${newStatus}`);
